    ...
    const key = e.which || e.keyCode;
              ...
              if(classNm == 'loan-want'){
                if (!isNumber && !isNumpadNumber && !controlKeys.includes(key)) {
                  if (isPrintable) showAlert('알림', '숫자만 입력해 주세요.');
...
//이름 한글만 입력되게
function onlyKorean(classNm){
  $('.'+classNm).on('input', function() {
      const input = $(this).val();
      const regex = /^[ㄱ-ㅎㅏ-ㅣ가-힣]*$/;

      if (!regex.test(input)) {
          showAlert('알림', '이름에는 한글만 입력해주세요.');
          $(this).val(input.replace(/[^ㄱ-ㅎㅏ-ㅣ가-힣]/g, ''));
      }
  });
}

//이메일 영문, 숫자, 특수문자만 입력되게
function onlyEmail(classNm){
  $('.'+classNm).on('input', function() {
      const input = $(this).val();
      const regex = /^[a-zA-Z0-9@._-]*$/; // 영문, 숫자, 특정 특수문자만 허용

      if (!regex.test(input)) {
          showAlert('알림', '이메일은 영문, 숫자, 특수문자(@ . _ -)만 입력해 주세요.');
